...
func TestPubKeyH(t *testing.T) {
	...
	pubKey, err := bmwire.NewPubKeyFromStr(pubKeyStr)
	if err != nil {
	...
	invalidHash := make([]byte, bmwire.PubKeySize+1)
	_, err = bmwire.NewPubKey(invalidHash)
	if err == nil {
...
func TestPubKeyString(t *testing.T) {
	...
	hash := bmwire.PubKey([bmwire.PubKeySize]byte{ // Make go vet happy.
...
func TestNewPubKeyFromStr(t *testing.T) {
	tests := []struct {
		...
		{
			"1",
			bmwire.PubKey([bmwire.PubKeySize]byte{ // Make go vet happy.
	...
	for i, test := range tests {
		result, err := bmwire.NewPubKeyFromStr(test.in)
		if err != test.err {
