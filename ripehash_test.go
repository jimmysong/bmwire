...
func TestRipeHash(t *testing.T) {
	...
	ripe, err := bmwire.NewRipeHashFromStr(ripeStr)
	if err != nil {
	...
	invalidHash := make([]byte, bmwire.RipeHashSize+1)
	_, err = bmwire.NewRipeHash(invalidHash)
	if err == nil {
...
func TestRipeHashString(t *testing.T) {
	...
	hash := bmwire.RipeHash([bmwire.RipeHashSize]byte{ // Make go vet happy.
...
func TestNewRipeHashFromStr(t *testing.T) {
	...
	for i, test := range tests {
		result, err := bmwire.NewRipeHashFromStr(test.in)
		if err != test.err {
